...
setMenuBar(false)
  ...
  return (
    <div className=" w-full ">
      <div className="flex flex-row font-semibold justify-between text-lg 
       md:px-24  px-10 bg-[#f97e27] shadow-[0_3px_10px_rgba(0,0,0,0.2)] z-[1000] ">
        ...
        <nav className=' hidden  md:flex gap-5  flex-row items-center justify-center md:gap-x-10 '>
          {menu.map((menuItem) => (
            <div key={menuItem.id} className={pathname === menuItem.href ? ' cursor-pointer bg-[#E3e3e3] h-full flex flex-row items-center justify-center px-5 text-primary' :
        ...
        <div className="hidden  md:flex flex-row text-[#E3e3e3] text-center  items-center justify-center ">
          <Link href={"/Search"} className="flex flex-row gap-x-2"> <Search/> Search</Link>
        </div>
      ...
      <div className={`${menuBar ? "translate-y-0 relative " : " -translate-y-full"} lg:hidden flex flex-col absolute
       bg-[#f97e27] text-[#E3e3e3] top-0  text-center h-fit w-full gap-4
       pt-4 pb-4  transition-all duration-300`}>
        {menu.map((menuItem) => (
          <div key={menuItem.id} className={pathname === menuItem.href ? 'py-2 cursor-pointer bg-[#E3e3e3]   items-center justify-center px-2 text-primary' :
